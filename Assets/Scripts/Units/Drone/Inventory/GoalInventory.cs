...
{
    public class GoalInventory : Inventory
    {
...
#pragma warning restore 649
        ...
        {
            if (!(item is GoalModule module)) return ItemAddResult.InvalidType;
            
            foreach (var slot in slots)
            {
                if (slot.IsEmpty || slot == item.GetCurrentSlot()) continue;
                var existingModule = (GoalModule)slot.GetItem();
        ...
        public override void OnSlotClicked(Slot slot, PointerEventData eventData)
        {
            ...
            if (slot.IsEmpty)
            {
                ...
                moduleText.text = "put a goal module here";
            }
            else
            {
